...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddToWishlist` (IN `memberID` INT, IN `productID` INT)   BEGIN
  INSERT INTO wishlist(memberID, productID, dateAdded) VALUES (memberID, productID, NOW());
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `CalculateTotalSuccessfulPayments` ()   BEGIN
    ...
    WHERE status = 'Success: Payment Successful, please check your emails for further details';
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `DeleteFromWishlist` (IN `P_memberID` INT, IN `P_productID` INT)   BEGIN
  DELETE FROM wishlist WHERE memberID = P_memberID AND productID = P_productID;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllPaymentsB` ()   BEGIN
    SELECT * FROM payment
    JOIN orders ON payment.orderID = orders.orderID
    WHERE payment.status = 'Success: Payment Successful, please check your emails for further details';
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetLastAddress` (IN `member_id` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetOrderDetailsB` (IN `order_id` INT)   BEGIN
    SELECT * FROM orderdetail WHERE orderID = order_id;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetWishlist` (IN `memberID` INT)   BEGIN
  SELECT wishlist.*, product.imagePath,product.price, product.productName 
  FROM wishlist 
  INNER JOIN product ON wishlist.productID = product.productID
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertPayment` (IN `p_orderID` INT, IN `p_paymentDate` DATETIME, IN `p_totalAmount` DECIMAL(10,2), IN `p_status` VARCHAR(255))   BEGIN
    INSERT INTO payment(orderID, paymentDate, totalAmount, status)
    VALUES (p_orderID, p_paymentDate, p_totalAmount, p_status);
END$$
...
(32, 'NewUser', 'NewUser', 'NewUser', 'newuser@gmail.com', 'a5d13922e14e42db74503183a2817593707c82398ebfe77f610a39a5f7190f8b', 'b10e95d5e5aed28e7f1888139bda023fb30ed6032fa3dde5048aa7bc67a58bd3', 'member'),
(33, 'finaltest', 'finaltest', 'finaltest', 'finaltest@gmail.com', '642e9bb974372a763f66a0590d2e487b1607e76a81fa54b36053d72780aa118b', '2516120bbdbd5ed8a0d09ffe40d31a7807e4ec9db97b36235fd6bdbc002225bd', 'member'),
(34, 'finalfinaltest', 'finalfinaltest', 'finalfinaltest', 'finalfinaltest@gmail.com', 'b8e8e7d4e0a080fb971e51ea42761bdeb969948bcd021320a2ccd85f60ad8515', '76167665a856914efb1c31eafa3ebb08c66ae0fd8fe7244cf9ed0d0beb1ea617', 'member'),
(35, 'Michael', 'Alfred', 'nigFredd', 'succefrss_fred@yahoo.com', '4c8419f78f2420ed446f20784721761978ecd331068eff49db0ba8fe28c2cd03', 'd73696b439e5351ac143f01218ffd7db4f7b6f01e7d97ee5b29c6724f9dcb6b7', 'member');
...
INSERT INTO `memberaddress` (`addressID`, `memberID`, `address1`, `address2`, `address3`, `city`, `county`, `eircode`) VALUES
(33, 33, 'Thomond Village, Old Cratloe Road', '', '', 'Limerick', 'Co. Limerick', 'V94 A0T0'),
(34, 31, 'Thomond Village, Old Cratloe Road', '', '', 'Limerick', 'Co. Limerick', 'V94 A0T0'),
(36, 34, 'Thomond Village, Old Cratloe Road', '', '', 'Limerick', 'Co. Limerick', 'V94 A0T0');
...
(5, 32, '2023-12-04 18:38:01', NULL, 'cart'),
(6, 33, '2023-12-04 19:39:10', NULL, 'cart'),
(7, 34, '2023-12-04 19:51:46', NULL, 'cart');
...
(3, 4, '2023-11-29 22:03:57', '26.88', 'Success: Payment Successful, please check your emails for further details'),
(4, 4, '2023-11-29 22:21:01', '26.88', 'Error: You cannot use a Stripe token more than once: tok_1OHvrODJTbFTfK1etVqtjnLC.'),
(5, 4, '2023-11-29 22:21:45', '26.88', 'Error: You cannot use a Stripe token more than once: tok_1OHvrODJTbFTfK1etVqtjnLC.'),
(6, 4, '2023-11-29 22:22:47', '26.88', 'Error: You cannot use a Stripe token more than once: tok_1OHvrODJTbFTfK1etVqtjnLC.'),
(7, 4, '2023-11-29 22:22:53', '26.88', 'Error: You cannot use a Stripe token more than once: tok_1OHvrODJTbFTfK1etVqtjnLC.'),
(8, 4, '2023-11-29 22:23:09', '26.88', 'Error: Your payment has already been processed. Please do not refresh the page.You cannot use a Stripe token more than once: tok_1OHvrODJTbFTfK1etVqtjnLC.'),
(9, 4, '2023-11-29 22:24:00', '26.88', 'Error: Your payment has already been processed. Please do not refresh the page.'),
(10, 4, '2023-11-29 22:29:54', '26.88', 'Error: Your payment has already been processed. Please do not refresh the page.'),
(11, 4, '2023-11-29 22:31:44', '26.88', 'Error: Your payment has already been processed. Please do not refresh the page.'),
(12, 4, '2023-11-29 22:44:13', '54.66', 'success'),
(13, 4, '2023-11-29 22:46:37', '54.66', 'Error: Your payment has already been processed. Please do not refresh the page.');
